...
export class SocketUseCases {
  ...
  async messageHandler(
    userId: number,
    receivedMessage: SendMessageDto
  ): Promise<any> {
    try {
      ...
      let conversation =
        await this.conversationUseCases.findOrCreateConversation([
          userId,
          recipient,
        ]);

      const message: Message = await this.messageBuilder(
        userId,
        receivedMessage,
        conversation
      );
  ...
  async togglePinMessage(
    ...
    const [conversation, message] =
      await this.conversationUseCases.findConversationAndMessage(
        messageId,
        userId,
        contactId
      );
    ...
    return await this.conversationUseCases.updateConversationMessage(
      conversation,
      message
    );
  }
  ...
  async messageBuilder(
    ...
    const bodyData:
      ...
      | PictureMessage
      | VoiceMessage
      | DocumentMessage = await this.saveMessage(
      messageBody,
  ...
  async saveMessage(messageBody: MessageBodyDto, pathArg: number[]) {
    ...
    const filePath = this.fileService.filePathGenerator(
      pathArg,
      messageBody.extName
    );
  ...
  checkSendMessageTerms(userId: number, message: SendMessageDto) {
    ...
    if (!recipient) throw new WsException("there is no user match this id");
